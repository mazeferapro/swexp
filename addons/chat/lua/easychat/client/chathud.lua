...
function chathud:Draw()
	...
	local hud_top = self.Pos.Y
	local hud_bottom = self.Pos.Y + self.Size.H

	self.DrawContext.DrawStart = RealTime()
	for _, line in ipairs(self.Lines) do
		-- skip lines that are completely outside the visible HUD area
		if line.Pos.Y + line.Size.H < hud_top or line.Pos.Y > hud_bottom then
			continue
		end
