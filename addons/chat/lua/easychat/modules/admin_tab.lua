...
if SERVER then
	...
	net.Receive(EASYCHAT_ADMIN, function(_, ply)
		...
		for _, p in ipairs(player.GetAll()) do
			if adminsg[p:GetUserGroup()] then
...
if CLIENT then
			...
			if not EasyChat.UseDermaSkin then
				...
				self.AdminList.Paint = function(self, w, h)
					...
					for _, ply in ipairs(player.GetAll()) do
						if adminsg[ply:GetUserGroup()] then
	...
	hook.Add("ECTabChanged", "EasyChatModuleDMTab", function(_, tab)
		if tab == "Админ" then
			...
			if not adminsg[LocalPlayer():GetUserGroup()] then
				EasyChat.AddText(self.RichText, "You cannot see the content of this channel because you are not an admin")
			end
		end
	end)

	EasyChat.AddMode("Админ", function(text)
		admintab:SendMessage(text)
	end)

	EasyChat.AddTab("Админ", admintab, "icon16/vcard.png")
	EasyChat.SetFocusForOn("Админ", admintab.TextEntry)

	hook.Add("ECOpened", "EasyChatModuleDMTab", function()
		local tab_data = EasyChat.GetTab("Админ")
		if tab_data and IsValid(tab_data.Tab) and IsValid(LocalPlayer()) then
			tab_data.Tab:SetVisible(adminsg[LocalPlayer():GetUserGroup()])
		end
	end)
end
