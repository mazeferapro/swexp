--[[local EASYCHAT_SRV = "EASY_CHAT_MODULE_SRV"
local EASYCHAT_SRV_REMOVE = "EASY_CHAT_MODULE_SRV_REMOVE"

if SERVER then
	util.AddNetworkString(EASYCHAT_SRV)
	util.AddNetworkString(EASYCHAT_SRV_REMOVE)

	net.Receive(EASYCHAT_SRV, function(_, ply)
	...
	local DM_TAB = {
		...
		CreateChat = function(self, ply)
			...
			if EC_HISTORY:GetBool() then
				...
				if EasyChat.IsStringEmpty(history) then
					EasyChat.AddText(richtext, "Это начало вашего диалога!\n\n")
				else
		...
		SendMessage = function(self, message)
			...
			if IsValid(ply) then
				...
				EasyChat.AddText(chat.RichText, "Этот игрок не на сервере :(")
			end
