...
local function DrawNoiseCompact()
    ...
    local hudCfg = (SWExp and SWExp.EnemyConfig and SWExp.EnemyConfig.HUD) or {}
    local lerpSp = hudCfg.lerpSpeed or 6
    STATE.noiseSmooth = Lerp(FrameTime() * lerpSp, STATE.noiseSmooth, STATE.noise)
    ...
    -- Respect alwaysVisible from config (default true). If false, hide when at/below stealth (closed eye state).
    local alwaysVis = (hudCfg.alwaysVisible ~= false)
    if not alwaysVis and frac <= stealthFrac then
        return
    end
