...
-- Получаем чат-сообщение без лимита 255 байт (обход TextMsg)
net.Receive("SWExp::ChatMsg", function()
...
function SWExp.F4:Open(tCharacters)
    ...
    function SWExp.F4:UpdateUI()
        ...
        if IsEmpty(char) then
            ...
            local isActiveChar = tonumber(char.id) == nActive
            -- donateModel приходит с сервера если у персонажа надет пак,
            -- заменяющий его текущую модель брони
            local _charModel = char.donateModel
                            or char.model
                            or 'models/player/combine_super_soldier.mdl'

            -- Для активного персонажа: берём живую модель с LocalPlayer()
            -- на случай если donateModel ещё не синхронизирован (сразу после экипировки)
            if isActiveChar and IsValid(LocalPlayer()) then
                local liveModel = LocalPlayer():GetModel()
                if liveModel and liveModel ~= '' then
                    _charModel = liveModel
                end
            end
            ...
            timer.Simple(0, function()
                ...
                if isActiveChar then
                    -- Активный: берём бодигруппы/скин прямо с живого игрока
                    -- (там уже применён донат-скин сервером)
                ...
                -- Неактивный персонаж: оригинальная логика без изменений.
                -- Если сервер не прислал bgData — пробуем LocalPlayer() при совпадении модели.
                if not bgData then
                    local ply = IsValid(LocalPlayer()) and LocalPlayer() or nil
    ...
    SGroup('Прокачка брони')
    SKeyBindRow('Меню прокачки', 'Открыть/закрыть меню прогрессии всех классов брони.', 'key_armor_prog', KEY_NONE)
end
