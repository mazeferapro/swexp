...
-- Net-канал для чата без лимита 255 байт (TextMsg)
util.AddNetworkString("SWExp::ChatMsg")

local function SendChatMsg(ply, msg)
    net.Start("SWExp::ChatMsg")
        net.WriteString(msg)
    net.Send(ply)
end
...
-- Обогащает список персонажей донат-моделью:
-- для каждого персонажа ищем надетый пак у которого replaces == char.model.
-- Если нашли — пишем char.donateModel = pack.model.
local function EnrichWithDonateModels(tChars, cb)
    local total = 0
    for _, char in ipairs(tChars) do
        if tonumber(char.id) ~= -1 then total = total + 1 end
    end

    if total == 0 then cb(tChars) return end

    local done = 0
    local function Advance()
        done = done + 1
        if done >= total then cb(tChars) end
    end

    for _, char in ipairs(tChars) do
        local charID = tonumber(char.id)
        if charID == -1 then
            -- виртуальный персонаж — пропускаем без счётчика
        else
            local charModel = char.model
            MySQLite.query(
                string.format('SELECT `item_id` FROM `swexp_donate_charmodel` WHERE `character_id` = %d;', charID),
                function(rows)
                    if rows and charModel and SWExp.DonateShop then
                        for _, r in ipairs(rows) do
                            local pack = SWExp.DonateShop:GetItem(r.item_id)
                            if pack and pack.type == 'model_pack'
                               and string.lower(pack.replaces) == string.lower(charModel) then
                                char.donateModel = pack.model
                                break
                            end
                        end
                    end
                ...
                function(err)
                    print('[SWExp.Chars] EnrichWithDonateModels err: ' .. tostring(err))
                    Advance()
                    return true
                end
            )
...
local function SendCharSelect(pPlayer, tChars)
    ...
    local function DoSend(enriched)
        -- Добавляем донат-модели для каждого персонажа
        EnrichWithDonateModels(enriched, function(final)
            if IsValid(pPlayer) then
                netstream.Start(pPlayer, 'SWExp::OpenCharSelect', final)
            end
        end)
    end
    ...
    if SWExp.Wardrobe and SWExp.Wardrobe.EnrichWithBodygroups then
        SWExp.Wardrobe.EnrichWithBodygroups(tChars, function(enriched)
            if IsValid(pPlayer) then DoSend(enriched) end
        end)
    else
        DoSend(tChars)
    end
...
hook.Add('PlayerSay', 'SWExp::ChatName', function(ply, text, teamChat)
    ...
    -- Пропускаем RP-команды (/me, /do, // и т.д.) — их обрабатывает SWExp::RpChatCommands
    local trimmedCheck = string.Trim(text)
    if string.sub(trimmedCheck, 1, 1) == "/" then return end
