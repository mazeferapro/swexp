...
local Colors = {
    ...
    -- /looc (локальный OOC)
    looc_bracket = Color(160, 130, 70),
    looc_name    = Color(190, 165, 110),
    looc_text    = Color(225, 210, 175),
...
net.Receive("SWExp::ChatCmd_Local", function()
    ...
    elseif cmdType == "looc" then
        -- (LOOC) Имя: текст
        chat.AddText(
            Colors.looc_bracket, "(LOOC) ",
            Colors.looc_name,    name .. ": ",
            Colors.looc_text,    text
        )
    end
end)
