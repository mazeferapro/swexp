...
local function CreateFonts()
    ...
    surface.CreateFont('SWUI.HUD.ArcCWMode', {
        font = 'Exo 2', size = S(17), weight = 800, extended = true,
    })
    surface.CreateFont('SWUI.HUD.ArcCWHeat', {
...
hook.Add('OnScreenSizeChanged', 'SWExp::RecreateFonts', CreateFonts)
hook.Add('InitPostEntity', 'SWExp::RecreateFontsOnInit', function()
    CreateFonts()
...
-- ============================================================
-- ARCCW: Режим стрельбы и перегрев
-- ============================================================

local function IsArcCWWeapon(wep)
    return IsValid(wep) and wep.ArcCW == true
end

local function DrawArcCWInfo()
    local ply = LocalPlayer()
    if not IsValid(ply) then return end
    local wep = ply:GetActiveWeapon()
    if not IsArcCWWeapon(wep) then return end

    -- Всё рисуем по центру экрана, под полоской HP
    -- HP bar bottom = ScrH() - S(56) + S(8) = ScrH() - S(48)
    local cx  = ScrW() / 2
    local topY = ScrH() - S(48)   -- нижний край HP бара

    -- === РЕЖИМ СТРЕЛЬБЫ ===
    local modeName = wep:GetFiremodeName()
    if modeName and modeName ~= "" then
        local modeUpper = string.upper(modeName)
        local modeColor
        if string.find(modeUpper, "AUTO") or string.find(modeUpper, "FULL") then
            modeColor = Color(0, 220, 80)
        elseif string.find(modeUpper, "SEMI") or string.find(modeUpper, "SINGLE") then
            modeColor = Color(0, 184, 255)
        elseif string.find(modeUpper, "BURST") then
            modeColor = Color(255, 180, 0)
        elseif string.find(modeUpper, "SAFE") then
            modeColor = Color(160, 160, 160)
        else
            modeColor = C.TextHi
        end

        Txt(string.upper(modeName), 'SWUI.HUD.ArcCWMode',
            cx, topY + S(6), modeColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
    end

    -- === ПЕРЕГРЕВ ===
    if not wep:HeatEnabled() then return end

    local heat       = wep:GetHeat()
    local heatMax    = wep:GetMaxHeat()
    local overheated = wep:GetHeatLocked()

    if not heatMax or heatMax <= 0 then return end

    local heatFrac = math.Clamp(heat / heatMax, 0, 1)

    local barW = S(200)
    local barH = S(4)
    local bx   = cx - barW / 2
    local by   = topY + S(26)   -- под текстом режима стрельбы

    -- Фоновая полоса
    draw.RoundedBox(S(2), bx, by, barW, barH, Color(255, 255, 255, 18))

    -- Заполнение: зелёный → жёлтый → красный
    local fr, fg
    if heatFrac < 0.5 then
        fr = math.Round(heatFrac * 2 * 255)
        fg = 220
    else
        fr = 255
        fg = math.Round((1 - (heatFrac - 0.5) * 2) * 220)
    end
    local fillCol = overheated and Color(255, 40, 40, 230) or Color(fr, fg, 0, 210)
    local fillW   = math.max(S(4), math.Round(barW * heatFrac))
    draw.RoundedBox(S(2), bx, by, fillW, barH, fillCol)

    -- Метка справа от бара
    local heatLabel = overheated and "!! ПЕРЕГРЕВ !!" or "ТЕПЛО"
    local labelCol  = overheated and Color(255, 80, 80) or C.TextDim
    Txt(heatLabel, 'SWUI.HUD.ArcCWHeat',
        cx, by + barH + S(3), labelCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
end

-- Отключаем стандартный DrawHUD у ArcCW оружий (затираем на экземпляре)
hook.Add("Think", "SWExp::DisableArcCWDrawHUD", function()
    local ply = LocalPlayer()
    if not IsValid(ply) then return end
    local wep = ply:GetActiveWeapon()
    if not IsValid(wep) or wep._SWExpHUDKilled then return end
    if IsArcCWWeapon(wep) then
        wep.DrawHUD = function() end
        wep._SWExpHUDKilled = true
    end
end)
