...
local function GetDisassemblableItems(ply)
    ...
    for uniqueID, item in pairs(stor.items) do
        ...
        if recipe then
            local refund = math.floor((recipe.cost or 0) / 2)
            table.insert(items, {
                uniqueID = uniqueID,
                itemID   = item.itemID,
...
net.Receive("SWExp::Disasm_Req", function(len, ply)
    ...
    if SWExp.Assembler then
        SWExp.Assembler._bank = (SWExp.Assembler._bank or 0) + refund
        if MySQLite then
            MySQLite.query(
    ...
    print(string.format("[SWExp|Disasm] %s разобрал «%s» ×%d → +%d мат. (банк=%d)",
        ply:Nick(), itemName, amount, refund, newBank))

    -- Результат — отправителю
    net.Start("SWExp::Disasm_Result")
        net.WriteBool(true)
